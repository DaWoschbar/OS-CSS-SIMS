    ...
    public class Incident : DBBase
    {
        ...
        public void Save()
        {
            using (NpgsqlConnection db = new NpgsqlConnection(base.ConnectionString))
            {
                ...
                if (Incident_id == 0)
                {
                    sql += $"INSERT INTO sims.incident(resolved, reporter, reported_at, description, title, resource_id, incident_type_id) ";
                    sql += $"VALUES (@resolved, @reporter, @reported_at, @description, @title, @resource_id, @incident_type_id);";
                }
                else
                {
                    sql += $"update sims.incident set resolved = @resolved, reporter = @reporter, reported_at = @reported_at, ";
                    sql += $"description = @description, title = @title, resource_id = @resource_id, incident_type_id = @incident_type_id where Incident_id = {Incident_id};";
                }
